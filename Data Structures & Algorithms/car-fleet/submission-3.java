class Solution {
    public int carFleet(int target, int[] position, int[] speed) {
        ...
        stack.add(time);
        for (int i = 1; i < n; i++) {
            // Calculate the time required for the current car and the one following it to reach the target position
            index = indices[i];
            time = (double) (target - position[index]) / speed[index];
            if (!stack.isEmpty() && time > stack.getLast()) {
                // If the car behind is faster, then they will form a fleet; therefore, we only need to keep the current car in the stack
                stack.add(time);
            }
        }
